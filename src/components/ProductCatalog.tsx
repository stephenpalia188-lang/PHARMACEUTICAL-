...
export const ProductCatalog: React.FC<ProductCatalogProps> = ({ onSelectProduct }) => {
  ...
  // Public catalog reads directly from Supabase. This avoids relying on /api/*
  // routes, which are not served by a static Vite deployment on Vercel.
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Keep the storefront usable if the production database has not been seeded yet.
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-xs mb-8 space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex rounded-xl bg-slate-100 p-1 shrink-0">
              <button
                ...
                className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer ${filterType === 'all' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
              >
                All Items
              </button>
              <button
                onClick={() => setFilterType('otc')}
                className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer ${filterType === 'otc' ? 'bg-emerald-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
              >
                OTC Only
              </button>
              <button
                onClick={() => setFilterType('rx')}
                className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer ${filterType === 'rx' ? 'bg-amber-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
              >
                Rx Required
              </button>
            </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-xs font-bold shrink-0 transition-all cursor-pointer ${selectedCategory === 'all' ? 'bg-emerald-800 text-white shadow-xs' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}`}
            >
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-3.5 py-1.5 rounded-full text-xs font-semibold shrink-0 transition-all cursor-pointer ${selectedCategory === cat.id ? 'bg-emerald-800 text-white shadow-xs font-bold' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}`}
              >
                {cat.name}
              </button>
            ))}
        ...
        {!loading && !error && filteredProducts.length === 0 && (
          <div className="rounded-2xl bg-white border border-slate-200 p-12 text-center max-w-md mx-auto space-y-4">
            ...
            <p className="text-xs text-slate-500">We couldn't find any products matching "{searchQuery}". You can check our full inventory or request a specialized order.</p>
        ...
        <div className="mt-12 p-4 rounded-xl bg-emerald-50/60 border border-emerald-200/80 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-emerald-900">
          <div className="flex items-center gap-3">
            ...
            <span><strong>Pharmacy Storage Guarantee:</strong> All thermolabile medications (insulin, vaccines, eye drops) are stored in continuous temperature-monitored pharmaceutical refrigerators.</span>
          </div>
          <span className="font-mono text-[11px] text-emerald-800 shrink-0">Kitale Town • Kijana Wamalwa Road</span>
        </div>
      </div>
    </section>
  );
